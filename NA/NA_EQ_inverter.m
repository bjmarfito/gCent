...
np    = sum(allnp);
% for k=1:np
%     [resampstruct(k).data]=-1*[resampstruct(k).data];
% %     [resampstruct(k).S]= 1*[resampstruct(k).S];
% end

data  = [resampstruct.data];
X     = [resampstruct.X];
Y     = [resampstruct.Y];
boxx  = [resampstruct.trix];
...
%     rampg  = blkdiag(rampg,[Xtmp'; Ytmp'; ones(1,allnp(i));XYtmp';XXtmp';YYtmp']);
    rampg   = blkdiag(rampg, [Xtmp; Ytmp; ones(1,allnp(i)); Xtmp.*Ytmp; Xtmp.^2.*Ytmp.^2]);
end
