...
n        = length(filename);
patterns = {'phsig','cor','unw','rdr','flat','hgt','dem','slc','crop'};
figtype  = {'phsig','rmg','rmg','rmg','cpx','rmg','dem','cpx','dem'};
...
elseif(regexp(filename,'crop'))
        [nx,ny,x1,y2,dx,dy] = loadGeoXml([filename '.xml']);
    nx = str2num(nx); ny = str2num(ny); x1 = str2num(x1); y2 = str2num(y2); dx = str2num(dx); dy = str2num(dy); lambda = [];
...
switch type
    case 'rmg'
        ...
        if(regexp(filename,'unw'));
            data            = phs*lambda/(4*pi); % Convention is positive = LOS increase (subsidence), negative = LOS decrease (uplift)
        end
    case 'cpx'
    ...
    case 'phsig'
        ...
        data = flipud(cor');
    case 'dem'
        disp('Loading DEM file')
        fid=fopen(filename,'r','native');
        [dem,count]=fread(fid,[nx,ny],'integer*2');
        data = flipud(dem');
end
